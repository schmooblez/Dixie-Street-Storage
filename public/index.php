<?php require "./../includes/_head.php"; ?>
<?php require "./../includes/_navigation.php" ?>
	<h2 id="pagename">Home&#58;</h2>
	<p class="description">Here at Dixie Street Storage, we take pride in our facility and believe that being family owned and operated ensures that we provide excellent customer service.</p>
	<div class="row">
		<div class="col-md-5" id="signdiv">
			<img class="images" src="/images/sign.jpg" alt="Dixie Street Storage Sign">
		</div>
		<div class="col-md-7" id="mapdiv">
			<iframe class="map" src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d6898.68109203555!2d-96.4224788809509!3d30.17026461818399!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x864402bbc1658455%3A0x40c8f277753e37c2!2s405+N+Dixie+St!5e0!3m2!1sen!2sus!4v1393213193484" width=98% height="285px" frameborder="1" alt="Dixie Street Storage Map"></iframe>
			<p class="center">405 N Dixie St Brenham, TX 77833</p>
			<p class="center">Located less than half a mile from the intersection of 290 and 36.</p>
		</div>
	</div>
	<div class="row" id="rowid">
		<div class="col-md-6" id="featuresdiv">
			<p class ="bold" id="features">Facility Features&#58;</p>
			<ul><p class ="bold" id="build">Buildings&#58;</p> <br />
				<li class="list">Clean Units for Immediate Move-in</li>
				<li class="list">Exterior Safety Lighting</li>
				<li class="list">Gated and Fenced</li>
			</ul>
			<ul><p class ="bold" id="access">Access&#58;</p> <br />
				<li class="list">Gated Access from 6 a.m. to 10 p.m.</li>
				<li class="list">Large Roll-up Doors for Easy Access</li>
				<li class="list">Paved Drives</li>
			</ul>
			<ul><p class ="bold" id="services">Services&#58;</p> <br />
				<li class="list">Convenient Office Hours (Mon&ndash;Fri; 9 AM&ndash;5 PM)</li>
				<li class="list">Weekend Appointments</li>
				<li class="list">RV, Boat, and Trailer Storage</li>
			</ul>
		</div>
		<div class="col-md-6">
			<img class="images" id="storage_units" src="/images/units.jpg" alt="Dixie Street Storage Units">
		</div>
	</div>
<?php require "./../includes/_footer.php"; ?>
</body>
</html>